...
void Panto::forwardKinematics()
{
    ...
    //TODO BIS: calculate position of elbows

    // Variables given:
    // leftBaseAngle is angle of motor left
    // rightBaseAngle is angle of motor right
    // c_leftBaseX, c_leftBaseY is position of left motor
    // c_rightBaseX, c_rightBaseY is position of right motor
    // c_leftInnerLength is length of left upper linkage
    // c_rightInnerLength is length of right upper linkage
    ...
    const auto leftElbowTotalAngle = leftElbowInsideAngle + leftElbowOffsetAngle;
    // PERFMON_STOP("[abbe] left elbow angles");

    // left elbow angle sin / cos
    // PERFMON_START("[abbf] left elbow angle sin / cos");
    const auto leftElbowTotalAngleSin = std::sin(leftElbowTotalAngle);
    const auto leftElbowTotalAngleCos = std::cos(leftElbowTotalAngle);
    // PERFMON_STOP("[abbf] left elbow angle sin / cos");

    // handle position
    // PERFMON_START("[abbg] handle position");
    m_handleX = fmaf(leftElbowTotalAngleCos, c_leftOuterLength, leftInnerX);
    m_handleY = fmaf(leftElbowTotalAngleSin, c_leftOuterLength, leftInnerY);
